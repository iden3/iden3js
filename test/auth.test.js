...
describe('[auth]new QR challenge (centralized website side)', () => {
  ...
  const kc0 = new iden3.KeyContainer(CONSTANTS.STORAGE.LOCAL_STORAGE.ID);
  kc0.unlock('pass');
...
describe('[auth] authorize the KSign (from the wallet side)', () => {
  ...
  it('[auth] id.AuthorizeKSignClaim()', () => id.createID().then((res) => {
    ...
    kc.unlock('pass');
    return id.authorizeKSignClaim(kc, id.keyOperational, 'iden3.io', qrKSign, 'appToAuthName', 'authz', unixtime, unixtime)
      .then((authorizeRes) => {
      // console.log(res.data);
        ...
        return iden3.auth.resolv(qrJson.url, id.idaddr, qrJson.challenge, qrJson.signature, qrKSign, ksignProof)
          .then((resolvRes) => {
            // console.log(res.data);
